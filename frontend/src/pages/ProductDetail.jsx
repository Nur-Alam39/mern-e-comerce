...
export default function ProductDetail() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // record recently viewed (store array of slugs, newest first)
        try {
          const key = 'recentlyViewed';
          const raw = localStorage.getItem(key);
          let arr = raw ? JSON.parse(raw) : [];
          // remove if already present
          arr = arr.filter(i => String(i) !== String(slug));
          arr.unshift(slug);
          // limit to 20
          arr = arr.slice(0, 20);
  ...
  return (
    <>
      ...
      <RelatedProducts categoryId={product.category ? (product.category._id || product.category) : null} currentSlug={product.slug} />
