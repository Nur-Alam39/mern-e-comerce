...
export default function FeaturedProducts({ limit = 8 }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await axios.get(`/api/products?featured=true&limit=${limit}`);
        const data = res && res.data ? res.data : [];
        // API returns either an array or a paginated object { products, page, pages, total }
        const items = Array.isArray(data) ? data : (data.products || []);
        setFeatured(items);
      } catch (err) {
        console.log('Failed to load featured products', err && err.message ? err.message : err);
      }
    };
    load();
  ...
  return (
    <section className="container py-4">
      <div className="d-flex align-items-center justify-content-between">
        ...
        <Link to="/products?featured=true" className="text-dark text-decoration-none">
          View All &nbsp;
